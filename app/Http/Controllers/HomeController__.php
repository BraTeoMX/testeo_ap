...
/*   foreach ($formData as $data) {
            ...
            $teamModulInfo = team_modulo::select('team_leader', 'modulo', 'sam')
                ->where('modulo', $modulo)
                ->first();


            $aux_team_leader = $teamModulInfo->team_leader;
            $aux_min_producidos=$piezas*$teamModulInfo->sam*1.02;
            $aux_proyeccion_minutos=  ($aux_min_producidos/$valor_hora)*$horas_laboradas;


            /************eficiencia ************** */

         /*  $tiempo_desocupacion = 630;
            $op_presencia = Team_Modulo::where('modulo', $modulo )->select('op_presencia','pxhrs','capacitacion','utility')->first();
            $min_presencia=$op_presencia->op_presencia*$tiempo_desocupacion;
            $min_presencia_netos=($min_presencia-($op_presencia->pxhrs+$op_presencia->capacitacion))+$op_presencia->utility;

            if($min_presencia_netos<>0)
                $aux_efic = ($aux_proyeccion_minutos/ $min_presencia_netos)*100;
            else
                $aux_efic = 0;

            Plan_diar::where('modulo', $modulo)->whereDate('fecha',$actual)->where('efic',0)->update([
                    'team_leader' => $aux_team_leader,
                    'min_producidos' => $aux_min_producidos,
                    'proyeccion_minutos' => $aux_proyeccion_minutos,
                    'efic' => $aux_efic
                ]);
...
/************************** por planta ************* */
        $plantas = Formato_P07::select('planta')->groupby('Planta')->get();
            ...
            ->where('planta','IntimarkI')->whereDate('ticket_offline.fecha',$actual)->sum('proyeccion_minutos');
            ...
            $op_presencia1_plantaII = Team_modulo::where('planta','IntimarkII')->where('modulo','<>','831A')->sum('op_presencia');
            $op_presencia2_plantaII = Team_modulo::where('planta','IntimarkII')->where('modulo','831A')->sum('op_presencia');

            $pxhrs_plantaII = Team_modulo::where('planta','IntimarkII')->where('modulo','<>','831A')->sum('pxhrs');
            $capacitacion_plantaII = Team_modulo::where('planta','IntimarkII')->where('modulo','<>','831A')->sum('capacitacion');
            $utility_plantaII = Team_modulo::where('planta','IntimarkII')->where('modulo','<>','831A')->sum('utility');
            $sam_plantaII = Team_Modulo::where('planta','IntimarkII')->where('modulo','<>','831A')->sum('sam');

            $cantidad_1_plantaII = Team_Modulo::select(DB::raw('sum(piezas_meta * sam) as total'))->where('planta','IntimarkII')->where('modulo','<>','831A')->get();
...
/************************** por cliente ************* */
        ...
        $cantidad_Empaque = Team_Modulo::where('cliente','Empaque')->whereDate('updated_at',$actual)->sum('min_x_producir');
       ...
       $min_pres_dia_Empaque = ($op_presencia1_dia_Empaque)* $tiempo_desocupacion;
        ...
        $proyeccion_min_VS = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','VS')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        ->where('cliente','CHICOS')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_BN3 = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','BN3')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_NU = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','NUUDS')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_MARENA = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','MARENA')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_PACIFIC = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','LEQ')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_BELL = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','BELL')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_WP = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','WP')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_HOOEY = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->where('cliente','HOOEY')->whereDate('ticket_offline.updated_at',$actual)->whereTime('ticket_offline.updated_at',$hora)->sum('proyeccion_minutos');
        ...
        $proyeccion_min_Empaque = Tickets_empaque::join('team_modulo','team_modulo.Modulo','ticket_empaque.modulo')
...
/**********************acumulado******************* */
  ...
  /*      for($i=1;$i<=$dia_semana;$i++){
        ...
        /*********** x modulos*********** */
        $modulos = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
        ->select('ticket_offline.modulo',  \DB::raw("SUM(ticket_offline.piezas) as piezas"))
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha','<=',$hora3)
        ->groupBy("ticket_offline.modulo")
        ->get();

        $modulosII = Plan_diar2::join('team_modulo','team_modulo.Modulo','planeacion_diaria.Modulo')
        ->select('planeacion_diaria.modulo',  \DB::raw("SUM(planeacion_diaria.piezas) as piezas"))
        ->whereDate('planeacion_diaria.updated_at',$actual)
        ->whereTime('planeacion_diaria.updated_at',$hora)
        ->groupBy("planeacion_diaria.modulo")
        ->get();
        ...
        $team_leader = Plan_diar::join('team_modulo','team_modulo.modulo','ticket_offline.modulo')
        ->select('ticket_offline.team_leader','team_modulo.planta', \DB::raw("SUM(ticket_offline.piezas) as piezas"), \DB::raw("SUM(ticket_offline.min_producidos) as min_producidos"),\DB::raw("SUM(ticket_offline.proyeccion_minutos) as proyeccion_minutos"), \DB::raw("AVG(ticket_offline.efic) as efic"), \DB::raw("SUM(team_modulo.piezas_meta) as piezas_meta"), \DB::raw("AVG(team_modulo.eficiencia_dia) as eficiencia_dia"))
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha','<=',$hora)
        ->groupby('ticket_offline.team_leader','team_modulo.planta')
        ->get();

        $team_leaderII = Plan_diar2::join('team_modulo','team_modulo.modulo','planeacion_diaria.modulo')
        ...
        $plantas = Plan_diar::join('team_modulo','team_modulo.modulo','ticket_offline.modulo')
       ...
       /******************planeacion ************* */
        ...
        $planeacion = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.team_leader','ticket_offline.Modulo','team_modulo.planta','team_modulo.cliente',\DB::raw("SUM(piezas) as piezas"))
        ->whereDate('ticket_offline.fecha',$actual)
        ->groupby('ticket_offline.modulo','ticket_offline.team_leader','team_modulo.cliente','team_modulo.planta')
        ->orderby('ticket_offline.team_leader','asc')
        ->orderby('ticket_offline.modulo','asc')
        ->get();

        $planeacionII = Plan_diar2::join('team_modulo','planeacion_diaria.Modulo','team_modulo.Modulo')
        ...
        $planeacion_09 = Plan_diar::select(\DB::raw("SUM(piezas) as piezas"),\DB::raw("SUM(min_producidos) as min_producidos"),\DB::raw("SUM(proyeccion_minutos) as proyeccion_minutos"), 'ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha','<=',"09:35")
        ->groupBy("Modulo")
        ->get();

        $planeacion_09II = Plan_diar2::join('team_modulo','planeacion_diaria.Modulo','team_modulo.Modulo')
        ...
        $planeacion_10 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"10:35")
        ->get();
        ...
        $planeacion_11 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"11:35")
        ->get();
        ...
        $planeacion_12 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"12:35")
        ->get();
        ...
        $planeacion_13 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"13:35")
        ->get();
        ...
        $planeacion_14 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"14:35")
        ->get();
        ...
        $planeacion_15 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"15:35")
        ->get();
        ...
        $planeacion_16 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"16:35")
        ->get();
        ...
        $planeacion_17 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"17:35")
        ->get();
        ...
        $planeacion_18 = Plan_diar::join('team_modulo','ticket_offline.Modulo','team_modulo.Modulo')
        ->select('ticket_offline.piezas','ticket_offline.min_producidos','ticket_offline.proyeccion_minutos', 'ticket_offline.efic','ticket_offline.Modulo')
        ->whereDate('ticket_offline.fecha',$actual)
        ->whereTime('ticket_offline.fecha',"18:35")
        ->get();
       ...
       $hora_actualiza = Plan_diar::whereDate('updated_at',$actual)->select('updated_at')->orderby('updated_at','desc')->first();
       ...
       ->select('ticket_offline.updated_at')->orderby('ticket_offline.updated_at','desc')->first();
       ...
       $proyeccion_min = Plan_diar::whereDate('updated_at',$actual)->whereTime('updated_at',$hora)->sum('proyeccion_minutos');

       if($hora_actualiza){
            $hora_actualizacion =date("H:i", strtotime($hora_actualiza->updated_at));

            if($hora3 <> $hora_actualizacion){
                $cantidad_diaria = Plan_diar::whereDate('updated_at',$actual)->whereTime('updated_at',$hora_actualizacion)->sum('piezas');
                $proyeccion_min = Plan_diar::whereDate('updated_at',$actual)->whereTime('updated_at',$hora_actualizacion)->sum('proyeccion_minutos');
            }
       }else{
       ...
       /*************real x planta ************ */
         ...
         //  ->whereTime('ticket_offline.fecha','>',$hora)
           ...
           /*************real x cliente ************ */
           $real_VS = Plan_diar::join('team_modulo','team_modulo.modulo','ticket_offline.modulo')
           ->where('team_modulo.cliente','VS')
           ->whereDate('ticket_offline.updated_at',$actual)
           ->whereTime('ticket_offline.updated_at','<=',$hora3)
           ->sum('piezas');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_VS = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','VS')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           $real_CHICOS = Plan_diar::join('team_modulo','team_modulo.modulo','ticket_offline.modulo')
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_CHICOS = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','CHICOS')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_BN3 = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','BN3')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_NU = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','NUUDS')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_MARENA = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','MARENA')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_PACIFIC = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','LEQ')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora3 <> $hora_actualizacion){
                ...
                $proyeccion_min_BELL = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','BELL')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
           ...
           if($hora <> $hora_actualizacion){
                ...
                $proyeccion_min_WP = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                ->where('cliente','WP')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
            ...
            if($hora3 <> $hora_actualizacion){
                 ...
                 $proyeccion_min_HOOEY = Plan_diar::join('team_modulo','team_modulo.Modulo','ticket_offline.Modulo')
                 ->where('cliente','HOOEY')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora_actualizacion)->sum('proyeccion_minutos');
            ...
            ->whereTime('ticket_empaque.fecha','<=',$hora3)
            ->sum('cantidad');
            ...
            if($hora3 <> $hora_actualizacion){
               $real_Empaque = Tickets_empaque::join('team_modulo','team_modulo.modulo','ticket_empaque.modulo') ///resultados por tickets
                ->where('team_modulo.cliente','Empaque')
                ->whereDate('ticket_empaque.fecha',$actual)
                ->whereTime('ticket_empaque.fecha','<=',$hora_actualizacion)
                //->sum('piezas');
                ->sum('cantidad');

                 $proyeccion_min_Empaque = Tickets_empaque::join('team_modulo','team_modulo.Modulo','ticket_empaque.modulo')
                 ->where('cliente','Empaque')->whereDate('ticket_empaque.fecha',$actual)->whereTime('ticket_empaque.fecha','<=',$hora_actualizacion)->sum('cantidad');
    ...
    public function detalleVS(request $request)
    {
         ...
         $dia_semana = date("N");
         $actual = date('Y-m-d');
         $hora_actualiza = Plan_diar::whereDate('fecha',$actual)->select('updated_at')->orderby('updated_at','desc')->first();
        ...
        ->where('cliente','VS')->whereDate('ticket_offline.fecha',$actual)->whereTime('ticket_offline.fecha','<=',$hora3)->where('planta','IntimarkI')->sum('proyeccion_minutos');
        ...
        ->where('cliente','Empaque')->whereDate('planeacion_diaria.updated_at',$actual)->whereTime('planeacion_diaria.updated_at',$hora)->where('planta','IntimarkII')->sum('cantidad');
        ...
        if($hora3 <> $hora_actualizacion){
         ...
         if($hora <> $hora_actualizacion){
              ...
              ->sum('cantidad');
                ...
                $proyeccion_min_EmpaqueII = Plan_diar2::join('team_modulo','team_modulo.Modulo','planeacion_diaria.modulo')
              ->where('cliente','Empaque')->whereDate('planeacion_diaria.updated_at',$actual)->whereTime('planeacion_diaria.updated_at',$hora_actualizacion)->where('planta','IntimarkII')->sum('cantidad');
